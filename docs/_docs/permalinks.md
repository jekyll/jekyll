...
<table>
  ...
  {%- for entry in site.data.permalinks.placeholders %}
    <tr>
      <td>
        <p><code>{{ entry.key }}</code></p>{% if entry.required_version %}
        <small>{% include docs_version_badge.html version=entry.required_version %}</small>{% endif %}
      </td>
      <td>
        <p>
          {{ entry.description }}{% if entry.override_key %}
          May be overridden via the document's <code>{{ entry.override_key }}</code> front matter.{% endif %}
        </p>
      </td>
    </tr>
  {%- endfor %}
