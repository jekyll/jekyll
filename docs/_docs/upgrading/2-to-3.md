...
An exception to the above rule are GitHub Pages sites, where the `--future` flag
remains *enabled* by default to maintain historical consistency for those sites.
