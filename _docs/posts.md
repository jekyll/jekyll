...
Note that the `post` variable is only available in the for loop. Use the `page` variable in other cases, such as in a template. The `page` variable is always available, no matter if you're dealing with pages, posts or drafts.
