...
class TestEntryFilter < JekyllUnitTest
  ...
  context "#glob_include?" do
    ...
    should "match directory only if there is trailing slash" do
      data = ["../source/_glob_include_test/_is_dir/", "../source/_glob_include_test/_not_dir/"]
      assert @filter.glob_include?(data, "../source/_glob_include_test/_is_dir")
      assert @filter.glob_include?(data, "../source/_glob_include_test/_is_dir/include_me.txt")
      refute @filter.glob_include?(data, "../source/_glob_include_test/_not_dir")
    end
  end
end
