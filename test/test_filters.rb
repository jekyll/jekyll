    ...
    context "group_by filter" do
      should "successfully group array of Jekyll::Page's" do
        ...
        grouping.each do |g|
          ...
          case g["name"]
          when "default"
            assert g["items"].is_a?(Array), "The list of grouping items for 'default' is not an Array."
            assert_equal 3, g["items"].size
          when "nil"
            assert g["items"].is_a?(Array), "The list of grouping items for 'nil' is not an Array."
            assert_equal 2, g["items"].size
          when ""
            assert g["items"].is_a?(Array), "The list of grouping items for '' is not an Array."
