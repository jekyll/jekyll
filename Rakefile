...
def hash_to_string(hash, indentation=0)
  ...
  hash.each do |key, value|
    ...
    string << if value.is_a? Hash
      ...
      ' ' << (value.nil? ? value.inspect : value.to_s)
    end
    string << "\n"
  end
  string
...
namespace :site do
  ...
  task :preview => [:default_configuration] do
    require "launchy"
  ...
  task :publish => [:history, :default_configuration] do
  ...
  desc "Generate a nicely formatted page for the default configuration"
  task :default_configuration do |t|
    defaults = Jekyll::Configuration[Jekyll::Configuration::DEFAULTS]

    defaults['source'] = '.'
    defaults['destination'] = File.basename(defaults['destination'])
    defaults['excerpt_separator'] = defaults['excerpt_separator'].inspect
    defaults['baseurl'] = defaults['baseurl'].inspect

    defaults = hash_to_string(defaults)
    Dir.chdir('site/_includes/') do
      File.open('default_configuration.html', 'w') do |file|
        file.write("{% comment %} This file is generated by `rake #{t}` {% endcomment %}\n")
        file.write(defaults)
      end
    end
  end
