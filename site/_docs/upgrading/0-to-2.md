...
use the subcommands `bundle exec jekyll build` and `bundle exec jekyll serve`
to build and preview your site.
...
<div class="note info">
  ...
  <p markdown="1">With the new subcommands, the way sites are previewed locally
   ...
   `watch: true`. Instead, use the `--watch` flag with either `bundle exec
   jekyll build` or `bundle exec jekyll serve`.</p>
</div>
