...
Each post can generate an excerpt by taking the text, from the beginning of the content
to the first occurrence of `excerpt_separator`, and sets it as the `post.excerpt`.
Take the above example of an index of posts. Perhaps you want to include
a little hint about the post's content by adding an excerpt of each of your
posts:
...
These tags can be removed with the following if you'd prefer:

{% highlight html %}
{% raw %}{{ post.excerpt | remove: '<p>' | remove: '</p>' }}{% endraw %}
{% endhighlight %}

Excerpts are off by default. To turn them on you can set the `excerpt_separator` or you can
add `excerpt` to your post's YAML front-matter.
